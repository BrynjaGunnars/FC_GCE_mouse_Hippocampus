...
For test run user needs to download the files within the CA2 directory 
(https://github.com/BrynjaGunnars/FC_GCE_mouse_Hippocampus/tree/main/test_data/CA2 ) 
and set the directory to the downloaded CA2 directory.
User additionally needs to set rightSide and indexCode below, further information below.
...
imgMaskRight = nib.load('geneFC_Mask_RIGHT.nii.gz')
...
imgMaskLeft = nib.load('geneFC_Mask_LEFT.nii.gz')
...
#set path to GCE maps

GCEpath = str(Path.cwd() / 'GCE_maps' / '*')

for name in glob.glob(GCEpath):
    
    #Need to set rightside to refer to x coordinates within GCE map filename
    #and indexCode to refer to x, y and z coordinates
    #example - for 6400_2200_5400_QBI_wholeMap_QBIparent_261259.nii.gz rightSide would be false and indexCode 261259
    
    #Right hemisphere x < 25, Left x > 25
    rightSide = (int(name[104:106]) < 25)
    indexCode = int(name[104:110])
